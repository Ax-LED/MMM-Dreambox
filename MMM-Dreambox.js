...
Module.register('MMM-Dreambox', {
	...
	defaults: {
		...
		apiBase: 'http://ipofyourreceiver:port',
		...
		apiLocations: '/web/getlocations', //where are the recording folders listet
		apiMovielist: '/web/movielist?dirname=' //link for opening recording folder and list recordings
	},
	
	// Define required scripts.
	getScripts: function() {
		//return ["moment.js", 'MMM-Dreambox-xml2json.js', "font-awesome.css"];//disabled, as we moved xml2json to node helper
	...
	getDom: function() {
		...
		Receiverinfo.innerHTML = this.translate("model");
	...
	servicelist2: function(liststart,listcondition){
		...
		for (let index = 0; index < this.anzahl; index++) {
			//console.log('Axled getelementbyid:',document.getElementById('MMM-Dreambox'+index)); 
			...
			if(this.mode === 'Recordings'){
				ServiceItem.innerHTML = index+1 +' '+ this.sender[index].e2title +', '+this.sender[index].e2description+' ('+this.sender[index].e2length+')';
			} else {
			...
			if((this.slp != undefined && this.timerstring != undefined && this.mode === '')||(this.slp != undefined && this.timerstring != null && this.mode === '')){
    ...
    notificationReceived: function(notification, payload) {
		...
		if(notification === "DB-STOP"){
			if (serviceselected !==''){
				...
				this.sendSocketNotification('FETCH_DATA', this.mode);
			}
		}

		if(notification === "DB-RECORDINGS") {
			//ToDo: um payload mit mode = recordings oder mode = services ergänzen
			this.updateDom();
			serviceselected = '';
 	...
 	socketNotificationReceived: function(notification, payload) {
		if (notification === "DATA") {
			...
			if(payload[0]==='DB-EPGNOW'){//no single Dom-Item to refresh, so storing as variable is fine
				...
				var json=payload[1];//as we received datas from node_helper in json-format
				...
				this.mode = '';
				this.servicelist2(this.liststart,this.listmax);//um das Dom mit neuen Daten zu füllen
			} else if(payload[0]==='DB-RECORDINGS'){//no single Dom-Item to refresh, so storing as variable is fine
				//var json=xml2json(payload[1]);
				var json=payload[1];//as we received datas from node_helper in json-format
				//console.log('Axled DB-RECORDINGS: ',moment().format('LTS')); 
				this.sender = json.e2movielist.e2movie;
				this.anzahl = json.e2movielist.e2movie.length;//Array und nicht Array abfangen....
				this.bouquet = payload[2];
				this.mode = 'Recordings';
				document.getElementById('bouquet').innerHTML = this.bouquet + ' (' + this.anzahl + ')';
				this.servicelist2(this.liststart,this.listmax);
			} 
